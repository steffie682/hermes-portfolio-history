...
async function inspectPdfInBrowser(source: Uint8Array): Promise<SafeReport> {
  const pdfjs = await import('pdfjs-dist/legacy/build/pdf.mjs');
  pdfjs.GlobalWorkerOptions.workerSrc = new URL(
    'pdfjs-dist/legacy/build/pdf.worker.min.mjs',
    import.meta.url,
  ).toString();
  const pages = await extractPdfStructure(source, pdfjs.getDocument as unknown as PdfDocumentLoader);
...
export default function SbiBalanceReportClient({
  ...
  inspectPdf?: (source: Uint8Array) => Promise<SafeReport>;
}) {
  ...
  async function handleFileChange(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const nextReport = await inspectPdf(bytes);
      if (version !== operationVersion.current) return;
